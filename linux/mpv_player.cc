...
void MpvPlayer::SetAspectRatioMode(const std::string& mode) {
  if (!mpv_) return;
  int keep = 1;
  double panscan = 0.0;
  if (mode == "fill") {
  ...
  mpv_set_property(mpv_, "keepaspect", MPV_FORMAT_FLAG, &keep);
  mpv_set_property(mpv_, "panscan", MPV_FORMAT_DOUBLE, &panscan);
  render_pending_.store(true, std::memory_order_release);
}

void MpvPlayer::SetVideoViewSize(uint32_t width, uint32_t height) {
  view_w_.store(width, std::memory_order_release);
  view_h_.store(height, std::memory_order_release);
  render_pending_.store(true, std::memory_order_release);
}

void MpvPlayer::EmitBuffering() {
  if (!on_buffering_) return;
  const bool buffering =
      seeking_.load(std::memory_order_relaxed) ||
      cache_paused_.load(std::memory_order_relaxed);
  on_buffering_(buffering);
}
    ...
    case MPV_EVENT_PLAYBACK_RESTART: {
      ...
      if (mpv_ && on_buffering_) {
        ...
        if (mpv_get_property(mpv_, "paused-for-cache", MPV_FORMAT_FLAG,
                             &cache_paused) >= 0) {
          cache_paused_.store(cache_paused != 0, std::memory_order_relaxed);
        }
        if (mpv_get_property(mpv_, "seeking", MPV_FORMAT_FLAG, &seeking) >= 0) {
          seeking_.store(seeking != 0, std::memory_order_relaxed);
        }
        EmitBuffering();
      }
    ...
    case MPV_EVENT_PROPERTY_CHANGE: {
      ...
      switch (ev->reply_userdata) {
        ...
        case kPropCache:
          if (prop->format == MPV_FORMAT_FLAG && prop->data) {
            int v = *static_cast<int*>(prop->data);
            cache_paused_.store(v != 0, std::memory_order_relaxed);
            EmitBuffering();
          }
          break;
        case kPropSeeking:
          if (prop->format == MPV_FORMAT_FLAG && prop->data) {
            int v = *static_cast<int*>(prop->data);
            seeking_.store(v != 0, std::memory_order_relaxed);
            EmitBuffering();
          }
          break;
...
bool MpvPlayer::Render() {
  ...
  // Prefer Flutter view size so keepaspect/panscan apply relative to the widget.
  int64_t w = static_cast<int64_t>(view_w_.load(std::memory_order_acquire));
  int64_t h = static_cast<int64_t>(view_h_.load(std::memory_order_acquire));
  if (w <= 0 || h <= 0) {
    // Prefer display dimensions (aspect-corrected), but fall back to raw source
    // dimensions if the VO hasn't populated dwidth/dheight yet.
    w = video_w_.load();
    h = video_h_.load();
    if (w <= 0 || h <= 0) {
      w = src_w_.load();
  ...
  // Cap SW rendering at 1080p on the long edge.
  ...
  // Align to a multiple of 4 on both axes.
  w = (w / 4) * 4;
  h = (h / 4) * 4;
  if (w < 4) w = 4;
  if (h < 4) h = 4;
