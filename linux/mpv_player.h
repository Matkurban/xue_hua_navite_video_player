...
#ifndef XUE_HUA_NAVITE_VIDEO_PLAYER_NATIVE_MPV_PLAYER_H_
#define XUE_HUA_NAVITE_VIDEO_PLAYER_NATIVE_MPV_PLAYER_H_
...
class MpvPlayer {
 public:
  ...
  // fit | fill | stretch — maps to keepaspect / panscan.
  void SetAspectRatioMode(const std::string& mode);
  // Logical Flutter view size in pixels (already multiplied by DPR if desired).
  // When >0, SW render targets this size so keepaspect/panscan apply to the view.
  void SetVideoViewSize(uint32_t width, uint32_t height);
  void Dispose();
 ...
 private:
  ...
  // Optional Flutter view size for aspect-correct SW rendering.
  std::atomic<uint32_t> view_w_{0};
  std::atomic<uint32_t> view_h_{0};

  std::atomic<bool> render_pending_{false};
  // Combined into on_buffering_: seeking || paused-for-cache.
  std::atomic<bool> seeking_{false};
  std::atomic<bool> cache_paused_{false};
...
#endif  // XUE_HUA_NAVITE_VIDEO_PLAYER_NATIVE_MPV_PLAYER_H_
